...
| Legacy session keys without slot suffix | `lib/integrations/openclaw/bootstrap-hook.ts` | `remove_now` | Removed parsing fallback for `project-role-level`; current sessions require `project-role-level-nameOrIndex`. |
| Old prompt paths | `lib/integrations/openclaw/bootstrap-hook.ts` | `remove_now` | Removed `projects/roles/*` prompt lookup. Current prompts live under `devclaw/prompts` or `devclaw/projects/<project>/prompts`. |
...
| Attachment hook top-level project `channelId` | `lib/application/tasks/attachment-hook.ts` | `remove_now` | Removed top-level `project.channelId` lookup. Current channel data is `project.channels[]`. |
