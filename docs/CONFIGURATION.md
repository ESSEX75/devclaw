...
Managed labels such as `step:<order>` and `blocked:step` are restored from `devclaw/sprints.json`. They describe the local graph in the provider UI; they do not drive dispatch decisions.
